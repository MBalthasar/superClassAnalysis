#' Resolution Saturation Identification Tool
#'
...
#'
#' @return A data.frame with the accuracy numbers per class depending on the spatial resolution.
#'
#' @examples
#' library(sp)
#' library(raster)
...
ResolutionSaturation <- function(img, model, trainData, valData, prodAcc, responseCol,
  ...
  # create data frame
  mat <- matrix(data = NA, nrow = length(resolutions), ncol = colnumber)
  acc.data <- as.data.frame(mat)
  # start for-loop for the number of different samples
  for (i in 1:length(resolutions)) {
    # create new raster with resolutions from vector
    new_res_img <- raster::projectRaster(from = img, res = resolutions[i],
                                 crs = crs(img), method = 'bilinear')
    # Beginn Classification with or without validation data (30% used for validation)
    if (missing(valData)) {
      set.seed(7)
      x <- RStoolbox::superClass(img = new_res_img, trainData = trainData, trainPartition = 0.7,
                      responseCol = responseCol, nSamples = nSamples,
                      areaWeightedSampling = TRUE, model = model,
                      mode = "classification", verbose = TRUE)
    } else {
      set.seed(7)
      x <- RStoolbox::superClass(img = new_res_img, trainData = trainData, valData = valData,
                      responseCol = responseCol, nSamples = nSamples,
                      areaWeightedSampling = TRUE, model = model,
                      mode = "classification", verbose = TRUE)
    }
    # Fill table with number of samples
    acc.data[i,1] <- resolutions[i]
  ...
  if (overall == TRUE) {
    names(acc.data) <- c("Resolution", "Overall_Accuracy", rownames)
  } else {
    names(acc.data) <- c("Resolution", rownames)
  }
  if (plot_graph == TRUE) {
    # melt data frame and plot results
    acc.data_long <- reshape2::melt(acc.data, id="Resolution")
    print(ggplot2::ggplot(data=acc.data_long, aes(x=Resolution, y=value, colour=variable)) +
            geom_line(size=1.05)+
            labs(y = "Accuracy", x = "Resolution", colour = "Legend:\n") +
